...
const SingleEvents = memo(({ filteredEvents, loadingFilter, selectedDate, lastEventElementRef }) => {
  ...
  return (
    <>
      {loadingFilter ? (
        ...
        selectedDate === "Bu Hafta" || selectedDate === "Bu Haftasonu" || selectedDate === "Önümüzdeki Hafta" ? (
          <>
        {!loadingFilter && Object.entries(groupedEvents).map(([date, events]) => (
            events.length > 0 ? (
              <div key={date}>
          ...
          <>
            {filteredEvents.length === 0 && <h1 className='fs-4' style={{ paddingLeft: "0.7rem" }}>Arama kriterinize göre bir etkinlik bulamadık</h1>}
            {filteredEvents.map((singleEvent, index) => (
              // Lazy loading eklenerek, sadece görünen bölüm yüklenecek
              <LazyLoadedEventCard
...
// LazyLoadedEventCard bileşeni, görünene kadar yüklenmeyi bekleyecek
